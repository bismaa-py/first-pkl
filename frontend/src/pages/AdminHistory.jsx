...
import { Search, History, Filter, Eye } from 'lucide-react';
...
export default function AdminHistory() {
  ...
  const fetchData = async () => {
    try {
      ...
      const url = '/peminjaman/history' + (params.length ? '?' + params.join('&') : '');
  ...
  const statusCounts = {
    ...
    konfirmasi_kembali: peminjamans.filter((p) => p.status === 'konfirmasi_kembali').length,
    ...
    <div className="page">
      <div className="page__header">
        <div>
          ...
          <p className="page__desc">Riwayat lengkap semua peminjaman barang</p>
        </div>
      </div>
      ...
      <div className="status-tabs">
        ...
        <button
          className={`status-tab ${filterStatus === 'dikembalikan' ? 'status-tab--active' : ''}`}
          onClick={() => setFilterStatus('dikembalikan')}
        >
          Dikembalikan ({statusCounts.dikembalikan})
        </button>
        <button
          className={`status-tab ${filterStatus === 'ditolak' ? 'status-tab--active' : ''}`}
              ...
              <table className="table">
                ...
                <tbody>
                  {filtered.map((p) => (
                    <tr key={p.id}>
                      <td>#{p.id}</td>
                      <td className="td-bold">{p.user?.nama || '-'}</td>
                      <td>{p.barang?.nama || '-'}</td>
                      <td>{p.jumlah}</td>
                      ...
                      <td>
                        {p.tanggal_dikembalikan
                          ? new Date(p.tanggal_dikembalikan).toLocaleDateString('id-ID')
                          : '-'}
                      </td>
